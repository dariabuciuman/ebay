...
function MyExpiredBids() {
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem("auth-token")) {
      ...
      axios({
        method: "get",
        url: `/api/private/user/expired-bids`,
        headers: {
          Authorization: jwt,
        },
      })
    ...
    <div>
      {user && (
        <div className="user-page">
          <h2 className="user-info">Your expired bids, {user.firstName}!</h2>
          {bids.length === 0 && <p className="list-text">You don't have any expired bids. </p>}
          {bids.length !== 0 && (
